...
# ============================================================
INPUT_PATH = 'Undersampling_85_15.csv'
OUTPUT_DIR = '.'
OUTPUT_PATH = os.path.join(OUTPUT_DIR, 'undersampled_85_15_naive_bayes_predictions.csv')
RANDOM_STATE = 42
...
# ============================================================
print("="*60)
print("STEP 1: Loading undersampled data")
print("="*60)
...
# Standardize categorical values so spaces/case differences do not become
# separate categories in the probability tables.
for feature in features:
    df[feature] = df[feature].astype(str).str.strip()

df['QuantityCat'] = df['QuantityCat'].str.lower()
df['UnitPriceCat'] = df['UnitPriceCat'].str.lower()
...
# ============================================================
# STEP 1B: SPREADSHEET-STYLE SUMMARY TABLES
# These match the summary-table style used in the sampling spreadsheets:
# Count, Yes, No, P(Yes), and P(No) for each category.
# ============================================================
print("\n" + "="*60)
print("STEP 1B: Spreadsheet-style summary figures")
print("="*60)


def print_summary_table(title, feature):
    print(f"\n--- {title.upper()} ---")
    print(f"{'Category':<25} {'Count':<10} {'Yes':<10} {'No':<10} {'P(Yes)':<15} {'P(No)':<15}")
    print("-" * 90)

    values = sorted(df[feature].unique().tolist(), key=lambda x: str(x))
    for value in values:
        subset = df[df[feature] == value]
        count = len(subset)
        yes = (subset[target] == 1).sum()
        no = (subset[target] == 0).sum()
        p_yes = yes / count if count > 0 else 0
        p_no = no / count if count > 0 else 0

        print(f"{str(value):<25} {count:<10} {yes:<10} {no:<10} {p_yes:<15.10f} {p_no:<15.10f}")


print_summary_table("Country", "Country")
print_summary_table("Quantity", "QuantityCat")
print_summary_table("Unit Cost", "UnitPriceCat")

print("\n--- IS CANCELLED ---")
print(f"{'Class':<25} {'Count':<10} {'Probability':<15}")
print("-" * 55)
cancel_counts = df[target].value_counts()
yes_count = cancel_counts.get(1, 0)
no_count = cancel_counts.get(0, 0)
total_count = len(df)
print(f"{'Yes':<25} {yes_count:<10} {yes_count / total_count:<15.10f}")
print(f"{'No':<25} {no_count:<10} {no_count / total_count:<15.10f}")
print(f"{'Total':<25} {total_count:<10}")

print_summary_table("Invoice Month", "InvoiceMonth")
print_summary_table("Invoice Hour", "InvoiceHour")
...
# ============================================================
print("\n" + "="*60)
print("STEP 2: Train/Test split on undersampled data")
print("="*60)
...
print("\nTest class distribution (kept unchanged):")
